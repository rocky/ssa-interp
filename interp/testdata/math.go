...
func TestNaN() {
	f64 := NaN()
	if f64 == f64 {
		panic(fmt.Sprintf("NaN() returns %g, expected NaN", f64))
	}
	f32 := float32(f64)
	if f32 == f32 {
		panic(fmt.Sprintf("float32(NaN()) is %g, expected NaN", f32))
	}
}
...
func TestAcos() {
	for i := 0; i < len(vf); i++ {
		...
		if f := Acos(a); !close(acos[i], f) {
			panic(fmt.Sprintf("Acos(%g) = %g, want %g", a, f, acos[i]))
		}
	}
	for i := 0; i < len(vfacosSC); i++ {
		if f := Acos(vfacosSC[i]); !alike(acosSC[i], f) {
			panic(fmt.Sprintf("Acos(%g) = %g, want %g", vfacosSC[i], f, acosSC[i]))
		}
	}
}
...
func TestAcosh() {
	for i := 0; i < len(vf); i++ {
		...
		if f := Acosh(a); !veryclose(acosh[i], f) {
			panic(fmt.Sprintf("Acosh(%g) = %g, want %g", a, f, acosh[i]))
		}
	}
	for i := 0; i < len(vfacoshSC); i++ {
		if f := Acosh(vfacoshSC[i]); !alike(acoshSC[i], f) {
			panic(fmt.Sprintf("Acosh(%g) = %g, want %g", vfacoshSC[i], f, acoshSC[i]))
		}
	}
}
...
func TestAsin() {
	for i := 0; i < len(vf); i++ {
		...
		if f := Asin(a); !veryclose(asin[i], f) {
			panic(fmt.Sprintf("Asin(%g) = %g, want %g", a, f, asin[i]))
		}
	}
	for i := 0; i < len(vfasinSC); i++ {
		if f := Asin(vfasinSC[i]); !alike(asinSC[i], f) {
			panic(fmt.Sprintf("Asin(%g) = %g, want %g", vfasinSC[i], f, asinSC[i]))
		}
	}
}
...
func TestAsinh() {
	for i := 0; i < len(vf); i++ {
		if f := Asinh(vf[i]); !veryclose(asinh[i], f) {
			panic(fmt.Sprintf("Asinh(%g) = %g, want %g", vf[i], f, asinh[i]))
		}
	}
	for i := 0; i < len(vfasinhSC); i++ {
		if f := Asinh(vfasinhSC[i]); !alike(asinhSC[i], f) {
			panic(fmt.Sprintf("Asinh(%g) = %g, want %g", vfasinhSC[i], f, asinhSC[i]))
		}
	}
}
...
func TestAtan() {
	for i := 0; i < len(vf); i++ {
		if f := Atan(vf[i]); !veryclose(atan[i], f) {
			panic(fmt.Sprintf("Atan(%g) = %g, want %g", vf[i], f, atan[i]))
		}
	}
	for i := 0; i < len(vfatanSC); i++ {
		if f := Atan(vfatanSC[i]); !alike(atanSC[i], f) {
			panic(fmt.Sprintf("Atan(%g) = %g, want %g", vfatanSC[i], f, atanSC[i]))
		}
	}
}
...
func TestAtanh() {
	for i := 0; i < len(vf); i++ {
		...
		if f := Atanh(a); !veryclose(atanh[i], f) {
			panic(fmt.Sprintf("Atanh(%g) = %g, want %g", a, f, atanh[i]))
		}
	}
	for i := 0; i < len(vfatanhSC); i++ {
		if f := Atanh(vfatanhSC[i]); !alike(atanhSC[i], f) {
			panic(fmt.Sprintf("Atanh(%g) = %g, want %g", vfatanhSC[i], f, atanhSC[i]))
		}
	}
}
...
func TestAtan2() {
	for i := 0; i < len(vf); i++ {
		if f := Atan2(10, vf[i]); !veryclose(atan2[i], f) {
			panic(fmt.Sprintf("Atan2(10, %g) = %g, want %g", vf[i], f, atan2[i]))
		}
	}
	for i := 0; i < len(vfatan2SC); i++ {
		if f := Atan2(vfatan2SC[i][0], vfatan2SC[i][1]); !alike(atan2SC[i], f) {
			panic(fmt.Sprintf("Atan2(%g, %g) = %g, want %g", vfatan2SC[i][0], vfatan2SC[i][1], f, atan2SC[i]))
		}
	}
}
...
func TestCbrt() {
	for i := 0; i < len(vf); i++ {
		if f := Cbrt(vf[i]); !veryclose(cbrt[i], f) {
			panic(fmt.Sprintf("Cbrt(%g) = %g, want %g", vf[i], f, cbrt[i]))
		}
	}
	for i := 0; i < len(vfcbrtSC); i++ {
		if f := Cbrt(vfcbrtSC[i]); !alike(cbrtSC[i], f) {
			panic(fmt.Sprintf("Cbrt(%g) = %g, want %g", vfcbrtSC[i], f, cbrtSC[i]))
		}
	}
}
...
func TestCeil() {
	for i := 0; i < len(vf); i++ {
		if f := Ceil(vf[i]); ceil[i] != f {
			panic(fmt.Sprintf("Ceil(%g) = %g, want %g", vf[i], f, ceil[i]))
		}
	}
	for i := 0; i < len(vfceilSC); i++ {
		if f := Ceil(vfceilSC[i]); !alike(ceilSC[i], f) {
			panic(fmt.Sprintf("Ceil(%g) = %g, want %g", vfceilSC[i], f, ceilSC[i]))
		}
	}
}
...
func TestCopysign() {
	for i := 0; i < len(vf); i++ {
		if f := Copysign(vf[i], -1); copysign[i] != f {
			panic(fmt.Sprintf("Copysign(%g, -1) = %g, want %g", vf[i], f, copysign[i]))
		}
	}
	for i := 0; i < len(vf); i++ {
		if f := Copysign(vf[i], 1); -copysign[i] != f {
	...
	for i := 0; i < len(vfcopysignSC); i++ {
		if f := Copysign(vfcopysignSC[i], -1); !alike(copysignSC[i], f) {
			panic(fmt.Sprintf("Copysign(%g, -1) = %g, want %g", vfcopysignSC[i], f, copysignSC[i]))
		}
	}
}
...
func TestCos() {
	for i := 0; i < len(vf); i++ {
		if f := Cos(vf[i]); !veryclose(cos[i], f) {
			panic(fmt.Sprintf("Cos(%g) = %g, want %g", vf[i], f, cos[i]))
		}
	}
	for i := 0; i < len(vfcosSC); i++ {
		if f := Cos(vfcosSC[i]); !alike(cosSC[i], f) {
			panic(fmt.Sprintf("Cos(%g) = %g, want %g", vfcosSC[i], f, cosSC[i]))
		}
	}
}
...
func TestCosh() {
	for i := 0; i < len(vf); i++ {
		if f := Cosh(vf[i]); !close(cosh[i], f) {
			panic(fmt.Sprintf("Cosh(%g) = %g, want %g", vf[i], f, cosh[i]))
		}
	}
	for i := 0; i < len(vfcoshSC); i++ {
		if f := Cosh(vfcoshSC[i]); !alike(coshSC[i], f) {
			panic(fmt.Sprintf("Cosh(%g) = %g, want %g", vfcoshSC[i], f, coshSC[i]))
		}
	}
}
...
func TestErf() {
	for i := 0; i < len(vf); i++ {
		...
		if f := Erf(a); !veryclose(erf[i], f) {
			panic(fmt.Sprintf("Erf(%g) = %g, want %g", a, f, erf[i]))
		}
	}
	for i := 0; i < len(vferfSC); i++ {
		if f := Erf(vferfSC[i]); !alike(erfSC[i], f) {
			panic(fmt.Sprintf("Erf(%g) = %g, want %g", vferfSC[i], f, erfSC[i]))
		}
	}
}
...
func TestErfc() {
	for i := 0; i < len(vf); i++ {
		...
		if f := Erfc(a); !veryclose(erfc[i], f) {
			panic(fmt.Sprintf("Erfc(%g) = %g, want %g", a, f, erfc[i]))
		}
	}
	for i := 0; i < len(vferfcSC); i++ {
		if f := Erfc(vferfcSC[i]); !alike(erfcSC[i], f) {
			panic(fmt.Sprintf("Erfc(%g) = %g, want %g", vferfcSC[i], f, erfcSC[i]))
		}
	}
}
...
func TestExpm1() {
	for i := 0; i < len(vf); i++ {
		...
		if f := Expm1(a); !veryclose(expm1[i], f) {
			panic(fmt.Sprintf("Expm1(%g) = %g, want %g", a, f, expm1[i]))
		}
	}
	for i := 0; i < len(vfexpm1SC); i++ {
		if f := Expm1(vfexpm1SC[i]); !alike(expm1SC[i], f) {
			panic(fmt.Sprintf("Expm1(%g) = %g, want %g", vfexpm1SC[i], f, expm1SC[i]))
		}
	}
}
...
func TestAbs() {
	for i := 0; i < len(vf); i++ {
		if f := Abs(vf[i]); fabs[i] != f {
			panic(fmt.Sprintf("Abs(%g) = %g, want %g", vf[i], f, fabs[i]))
		}
	}
	for i := 0; i < len(vffabsSC); i++ {
		if f := Abs(vffabsSC[i]); !alike(fabsSC[i], f) {
			panic(fmt.Sprintf("Abs(%g) = %g, want %g", vffabsSC[i], f, fabsSC[i]))
		}
	}
}
...
func TestDim() {
	for i := 0; i < len(vf); i++ {
		if f := Dim(vf[i], 0); fdim[i] != f {
			panic(fmt.Sprintf("Dim(%g, %g) = %g, want %g", vf[i], 0.0, f, fdim[i]))
		}
	}
	for i := 0; i < len(vffdimSC); i++ {
		if f := Dim(vffdimSC[i][0], vffdimSC[i][1]); !alike(fdimSC[i], f) {
			panic(fmt.Sprintf("Dim(%g, %g) = %g, want %g", vffdimSC[i][0], vffdimSC[i][1], f, fdimSC[i]))
		}
	}
}
...
func TestFloor() {
	for i := 0; i < len(vf); i++ {
		if f := Floor(vf[i]); floor[i] != f {
			panic(fmt.Sprintf("Floor(%g) = %g, want %g", vf[i], f, floor[i]))
		}
	}
	for i := 0; i < len(vfceilSC); i++ {
		if f := Floor(vfceilSC[i]); !alike(ceilSC[i], f) {
			panic(fmt.Sprintf("Floor(%g) = %g, want %g", vfceilSC[i], f, ceilSC[i]))
		}
	}
}
...
func TestMax() {
	for i := 0; i < len(vf); i++ {
		if f := Max(vf[i], ceil[i]); ceil[i] != f {
			panic(fmt.Sprintf("Max(%g, %g) = %g, want %g", vf[i], ceil[i], f, ceil[i]))
		}
	}
	for i := 0; i < len(vffdimSC); i++ {
		if f := Max(vffdimSC[i][0], vffdimSC[i][1]); !alike(fmaxSC[i], f) {
			panic(fmt.Sprintf("Max(%g, %g) = %g, want %g", vffdimSC[i][0], vffdimSC[i][1], f, fmaxSC[i]))
		}
	}
}
...
func TestMin() {
	for i := 0; i < len(vf); i++ {
		if f := Min(vf[i], floor[i]); floor[i] != f {
			panic(fmt.Sprintf("Min(%g, %g) = %g, want %g", vf[i], floor[i], f, floor[i]))
		}
	}
	for i := 0; i < len(vffdimSC); i++ {
		if f := Min(vffdimSC[i][0], vffdimSC[i][1]); !alike(fminSC[i], f) {
			panic(fmt.Sprintf("Min(%g, %g) = %g, want %g", vffdimSC[i][0], vffdimSC[i][1], f, fminSC[i]))
		}
	}
}
...
func TestMod() {
	for i := 0; i < len(vf); i++ {
		if f := Mod(10, vf[i]); fmod[i] != f {
			panic(fmt.Sprintf("Mod(10, %g) = %g, want %g", vf[i], f, fmod[i]))
		}
	}
	for i := 0; i < len(vffmodSC); i++ {
		if f := Mod(vffmodSC[i][0], vffmodSC[i][1]); !alike(fmodSC[i], f) {
			panic(fmt.Sprintf("Mod(%g, %g) = %g, want %g", vffmodSC[i][0], vffmodSC[i][1], f, fmodSC[i]))
		}
	}
}
...
func TestFrexp() {
	for i := 0; i < len(vf); i++ {
		if f, j := Frexp(vf[i]); !veryclose(frexp[i].f, f) || frexp[i].i != j {
			panic(fmt.Sprintf("Frexp(%g) = %g, %d, want %g, %d", vf[i], f, j, frexp[i].f, frexp[i].i))
		}
	}
	for i := 0; i < len(vffrexpSC); i++ {
		if f, j := Frexp(vffrexpSC[i]); !alike(frexpSC[i].f, f) || frexpSC[i].i != j {
			panic(fmt.Sprintf("Frexp(%g) = %g, %d, want %g, %d", vffrexpSC[i], f, j, frexpSC[i].f, frexpSC[i].i))
		}
	}
	for i := 0; i < len(vffrexpBC); i++ {
		if f, j := Frexp(vffrexpBC[i]); !alike(frexpBC[i].f, f) || frexpBC[i].i != j {
			panic(fmt.Sprintf("Frexp(%g) = %g, %d, want %g, %d", vffrexpBC[i], f, j, frexpBC[i].f, frexpBC[i].i))
		}
	}
}
...
func TestGamma() {
	for i := 0; i < len(vf); i++ {
		if f := Gamma(vf[i]); !close(gamma[i], f) {
			panic(fmt.Sprintf("Gamma(%g) = %g, want %g", vf[i], f, gamma[i]))
		}
	}
	for i := 0; i < len(vfgammaSC); i++ {
		if f := Gamma(vfgammaSC[i]); !alike(gammaSC[i], f) {
			panic(fmt.Sprintf("Gamma(%g) = %g, want %g", vfgammaSC[i], f, gammaSC[i]))
		}
	}
}
...
func TestHypot() {
	for i := 0; i < len(vf); i++ {
		...
		if f := Hypot(1e200*tanh[i], 1e200*tanh[i]); !veryclose(a, f) {
			panic(fmt.Sprintf("Hypot(%g, %g) = %g, want %g", 1e200*tanh[i], 1e200*tanh[i], f, a))
		}
	}
	for i := 0; i < len(vfhypotSC); i++ {
		if f := Hypot(vfhypotSC[i][0], vfhypotSC[i][1]); !alike(hypotSC[i], f) {
			panic(fmt.Sprintf("Hypot(%g, %g) = %g, want %g", vfhypotSC[i][0], vfhypotSC[i][1], f, hypotSC[i]))
		}
	}
}
...
func TestIlogb() {
	for i := 0; i < len(vf); i++ {
		...
		if e := Ilogb(vf[i]); a != e {
			panic(fmt.Sprintf("Ilogb(%g) = %d, want %d", vf[i], e, a))
		}
	}
	for i := 0; i < len(vflogbSC); i++ {
		if e := Ilogb(vflogbSC[i]); ilogbSC[i] != e {
			panic(fmt.Sprintf("Ilogb(%g) = %d, want %d", vflogbSC[i], e, ilogbSC[i]))
		}
	}
	for i := 0; i < len(vffrexpBC); i++ {
		if e := Ilogb(vffrexpBC[i]); int(logbBC[i]) != e {
			panic(fmt.Sprintf("Ilogb(%g) = %d, want %d", vffrexpBC[i], e, int(logbBC[i])))
		}
	}
}
...
func TestJ0() {
	for i := 0; i < len(vf); i++ {
		if f := J0(vf[i]); !soclose(j0[i], f, 4e-14) {
			panic(fmt.Sprintf("J0(%g) = %g, want %g", vf[i], f, j0[i]))
		}
	}
	for i := 0; i < len(vfj0SC); i++ {
		if f := J0(vfj0SC[i]); !alike(j0SC[i], f) {
			panic(fmt.Sprintf("J0(%g) = %g, want %g", vfj0SC[i], f, j0SC[i]))
		}
	}
}
...
func TestJ1() {
	for i := 0; i < len(vf); i++ {
		if f := J1(vf[i]); !close(j1[i], f) {
			panic(fmt.Sprintf("J1(%g) = %g, want %g", vf[i], f, j1[i]))
		}
	}
	for i := 0; i < len(vfj0SC); i++ {
		if f := J1(vfj0SC[i]); !alike(j1SC[i], f) {
			panic(fmt.Sprintf("J1(%g) = %g, want %g", vfj0SC[i], f, j1SC[i]))
		}
	}
}
...
func TestJn() {
	for i := 0; i < len(vf); i++ {
		if f := Jn(2, vf[i]); !close(j2[i], f) {
			panic(fmt.Sprintf("Jn(2, %g) = %g, want %g", vf[i], f, j2[i]))
		}
		if f := Jn(-3, vf[i]); !close(jM3[i], f) {
			panic(fmt.Sprintf("Jn(-3, %g) = %g, want %g", vf[i], f, jM3[i]))
		}
	}
	for i := 0; i < len(vfj0SC); i++ {
		if f := Jn(2, vfj0SC[i]); !alike(j2SC[i], f) {
			panic(fmt.Sprintf("Jn(2, %g) = %g, want %g", vfj0SC[i], f, j2SC[i]))
		}
		if f := Jn(-3, vfj0SC[i]); !alike(jM3SC[i], f) {
			panic(fmt.Sprintf("Jn(-3, %g) = %g, want %g", vfj0SC[i], f, jM3SC[i]))
		}
	}
}
...
func TestLdexp() {
	for i := 0; i < len(vf); i++ {
		if f := Ldexp(frexp[i].f, frexp[i].i); !veryclose(vf[i], f) {
			panic(fmt.Sprintf("Ldexp(%g, %d) = %g, want %g", frexp[i].f, frexp[i].i, f, vf[i]))
		}
	}
	for i := 0; i < len(vffrexpSC); i++ {
		if f := Ldexp(frexpSC[i].f, frexpSC[i].i); !alike(vffrexpSC[i], f) {
			panic(fmt.Sprintf("Ldexp(%g, %d) = %g, want %g", frexpSC[i].f, frexpSC[i].i, f, vffrexpSC[i]))
		}
	}
	for i := 0; i < len(vfldexpSC); i++ {
		if f := Ldexp(vfldexpSC[i].f, vfldexpSC[i].i); !alike(ldexpSC[i], f) {
			panic(fmt.Sprintf("Ldexp(%g, %d) = %g, want %g", vfldexpSC[i].f, vfldexpSC[i].i, f, ldexpSC[i]))
		}
	}
	for i := 0; i < len(vffrexpBC); i++ {
		if f := Ldexp(frexpBC[i].f, frexpBC[i].i); !alike(vffrexpBC[i], f) {
			panic(fmt.Sprintf("Ldexp(%g, %d) = %g, want %g", frexpBC[i].f, frexpBC[i].i, f, vffrexpBC[i]))
		}
	}
	for i := 0; i < len(vfldexpBC); i++ {
		if f := Ldexp(vfldexpBC[i].f, vfldexpBC[i].i); !alike(ldexpBC[i], f) {
			panic(fmt.Sprintf("Ldexp(%g, %d) = %g, want %g", vfldexpBC[i].f, vfldexpBC[i].i, f, ldexpBC[i]))
		}
	}
}
...
func TestLgamma() {
	for i := 0; i < len(vf); i++ {
		if f, s := Lgamma(vf[i]); !close(lgamma[i].f, f) || lgamma[i].i != s {
			panic(fmt.Sprintf("Lgamma(%g) = %g, %d, want %g, %d", vf[i], f, s, lgamma[i].f, lgamma[i].i))
		}
	}
	for i := 0; i < len(vflgammaSC); i++ {
		if f, s := Lgamma(vflgammaSC[i]); !alike(lgammaSC[i].f, f) || lgammaSC[i].i != s {
			panic(fmt.Sprintf("Lgamma(%g) = %g, %d, want %g, %d", vflgammaSC[i], f, s, lgammaSC[i].f, lgammaSC[i].i))
		}
	}
}
...
func TestLog() {
	for i := 0; i < len(vf); i++ {
		a := Abs(vf[i])
		if f := Log(a); log[i] != f {
			panic(fmt.Sprintf("Log(%g) = %g, want %g", a, f, log[i]))
		}
	}
	if f := Log(10); f != Ln10 {
		panic(fmt.Sprintf("Log(%g) = %g, want %g", 10.0, f, Ln10))
	}
	for i := 0; i < len(vflogSC); i++ {
		if f := Log(vflogSC[i]); !alike(logSC[i], f) {
			panic(fmt.Sprintf("Log(%g) = %g, want %g", vflogSC[i], f, logSC[i]))
		}
	}
}
...
func TestLogb() {
	for i := 0; i < len(vf); i++ {
		if f := Logb(vf[i]); logb[i] != f {
			panic(fmt.Sprintf("Logb(%g) = %g, want %g", vf[i], f, logb[i]))
		}
	}
	for i := 0; i < len(vflogbSC); i++ {
		if f := Logb(vflogbSC[i]); !alike(logbSC[i], f) {
			panic(fmt.Sprintf("Logb(%g) = %g, want %g", vflogbSC[i], f, logbSC[i]))
		}
	}
	for i := 0; i < len(vffrexpBC); i++ {
		if f := Logb(vffrexpBC[i]); !alike(logbBC[i], f) {
			panic(fmt.Sprintf("Logb(%g) = %g, want %g", vffrexpBC[i], f, logbBC[i]))
		}
	}
}
...
func TestLog10() {
	for i := 0; i < len(vf); i++ {
		...
		if f := Log10(a); !veryclose(log10[i], f) {
			panic(fmt.Sprintf("Log10(%g) = %g, want %g", a, f, log10[i]))
		}
	}
	if f := Log10(E); f != Log10E {
		panic(fmt.Sprintf("Log10(%g) = %g, want %g", E, f, Log10E))
	}
	for i := 0; i < len(vflogSC); i++ {
		if f := Log10(vflogSC[i]); !alike(logSC[i], f) {
			panic(fmt.Sprintf("Log10(%g) = %g, want %g", vflogSC[i], f, logSC[i]))
		}
	}
}
...
func TestLog1p() {
	for i := 0; i < len(vf); i++ {
		...
		if f := Log1p(a); !veryclose(log1p[i], f) {
			panic(fmt.Sprintf("Log1p(%g) = %g, want %g", a, f, log1p[i]))
		}
	}
	a := 9.0
	if f := Log1p(a); f != Ln10 {
		panic(fmt.Sprintf("Log1p(%g) = %g, want %g", a, f, Ln10))
	}
	for i := 0; i < len(vflogSC); i++ {
		if f := Log1p(vflog1pSC[i]); !alike(log1pSC[i], f) {
			panic(fmt.Sprintf("Log1p(%g) = %g, want %g", vflog1pSC[i], f, log1pSC[i]))
		}
	}
}
...
func TestLog2() {
	for i := 0; i < len(vf); i++ {
		...
		if f := Log2(a); !veryclose(log2[i], f) {
			panic(fmt.Sprintf("Log2(%g) = %g, want %g", a, f, log2[i]))
		}
	}
	if f := Log2(E); f != Log2E {
		panic(fmt.Sprintf("Log2(%g) = %g, want %g", E, f, Log2E))
	}
	for i := 0; i < len(vflogSC); i++ {
		if f := Log2(vflogSC[i]); !alike(logSC[i], f) {
			panic(fmt.Sprintf("Log2(%g) = %g, want %g", vflogSC[i], f, logSC[i]))
		}
	}
	for i := -1074; i <= 1023; i++ {
		f := Ldexp(1, i)
		l := Log2(f)
		if l != float64(i) {
			panic(fmt.Sprintf("Log2(2**%d) = %g, want %d", i, l, i))
		}
	}
}
...
func TestModf() {
	for i := 0; i < len(vf); i++ {
		if f, g := Modf(vf[i]); !veryclose(modf[i][0], f) || !veryclose(modf[i][1], g) {
			panic(fmt.Sprintf("Modf(%g) = %g, %g, want %g, %g", vf[i], f, g, modf[i][0], modf[i][1]))
		}
	}
	for i := 0; i < len(vfmodfSC); i++ {
		if f, g := Modf(vfmodfSC[i]); !alike(modfSC[i][0], f) || !alike(modfSC[i][1], g) {
			panic(fmt.Sprintf("Modf(%g) = %g, %g, want %g, %g", vfmodfSC[i], f, g, modfSC[i][0], modfSC[i][1]))
		}
	}
}
...
func TestNextafter() {
	for i := 0; i < len(vf); i++ {
		if f := Nextafter(vf[i], 10); nextafter[i] != f {
			panic(fmt.Sprintf("Nextafter(%g, %g) = %g want %g", vf[i], 10.0, f, nextafter[i]))
		}
	}
	for i := 0; i < len(vfnextafterSC); i++ {
		if f := Nextafter(vfnextafterSC[i][0], vfnextafterSC[i][1]); !alike(nextafterSC[i], f) {
			panic(fmt.Sprintf("Nextafter(%g, %g) = %g want %g", vfnextafterSC[i][0], vfnextafterSC[i][1], f, nextafterSC[i]))
		}
	}
}
...
func TestPow() {
	for i := 0; i < len(vf); i++ {
		if f := Pow(10, vf[i]); !close(pow[i], f) {
			panic(fmt.Sprintf("Pow(10, %g) = %g, want %g", vf[i], f, pow[i]))
		}
	}
	for i := 0; i < len(vfpowSC); i++ {
		if f := Pow(vfpowSC[i][0], vfpowSC[i][1]); !alike(powSC[i], f) {
			panic(fmt.Sprintf("Pow(%g, %g) = %g, want %g", vfpowSC[i][0], vfpowSC[i][1], f, powSC[i]))
		}
	}
}
...
func TestPow10() {
	for i := 0; i < len(vfpow10SC); i++ {
		if f := Pow10(vfpow10SC[i]); !alike(pow10SC[i], f) {
			panic(fmt.Sprintf("Pow10(%d) = %g, want %g", vfpow10SC[i], f, pow10SC[i]))
		}
	}
}
...
func TestRemainder() {
	for i := 0; i < len(vf); i++ {
		if f := Remainder(10, vf[i]); remainder[i] != f {
			panic(fmt.Sprintf("Remainder(10, %g) = %g, want %g", vf[i], f, remainder[i]))
		}
	}
	for i := 0; i < len(vffmodSC); i++ {
		if f := Remainder(vffmodSC[i][0], vffmodSC[i][1]); !alike(fmodSC[i], f) {
			panic(fmt.Sprintf("Remainder(%g, %g) = %g, want %g", vffmodSC[i][0], vffmodSC[i][1], f, fmodSC[i]))
		}
	}
}
...
func TestSignbit() {
	for i := 0; i < len(vf); i++ {
		if f := Signbit(vf[i]); signbit[i] != f {
			panic(fmt.Sprintf("Signbit(%g) = %t, want %t", vf[i], f, signbit[i]))
		}
	}
	for i := 0; i < len(vfsignbitSC); i++ {
		if f := Signbit(vfsignbitSC[i]); signbitSC[i] != f {
			panic(fmt.Sprintf("Signbit(%g) = %t, want %t", vfsignbitSC[i], f, signbitSC[i]))
		}
	}
}
func TestSin() {
	for i := 0; i < len(vf); i++ {
		if f := Sin(vf[i]); !veryclose(sin[i], f) {
			panic(fmt.Sprintf("Sin(%g) = %g, want %g", vf[i], f, sin[i]))
		}
	}
	for i := 0; i < len(vfsinSC); i++ {
		if f := Sin(vfsinSC[i]); !alike(sinSC[i], f) {
			panic(fmt.Sprintf("Sin(%g) = %g, want %g", vfsinSC[i], f, sinSC[i]))
		}
	}
}
...
func TestSincos() {
	for i := 0; i < len(vf); i++ {
		if s, c := Sincos(vf[i]); !veryclose(sin[i], s) || !veryclose(cos[i], c) {
			panic(fmt.Sprintf("Sincos(%g) = %g, %g want %g, %g", vf[i], s, c, sin[i], cos[i]))
		}
	}
}
...
func TestSinh() {
	for i := 0; i < len(vf); i++ {
		if f := Sinh(vf[i]); !close(sinh[i], f) {
			panic(fmt.Sprintf("Sinh(%g) = %g, want %g", vf[i], f, sinh[i]))
		}
	}
	for i := 0; i < len(vfsinhSC); i++ {
		if f := Sinh(vfsinhSC[i]); !alike(sinhSC[i], f) {
			panic(fmt.Sprintf("Sinh(%g) = %g, want %g", vfsinhSC[i], f, sinhSC[i]))
		}
	}
}
...
func TestTan() {
	for i := 0; i < len(vf); i++ {
		if f := Tan(vf[i]); !veryclose(tan[i], f) {
			panic(fmt.Sprintf("Tan(%g) = %g, want %g", vf[i], f, tan[i]))
		}
	}
	// same special cases as Sin
	for i := 0; i < len(vfsinSC); i++ {
		if f := Tan(vfsinSC[i]); !alike(sinSC[i], f) {
			panic(fmt.Sprintf("Tan(%g) = %g, want %g", vfsinSC[i], f, sinSC[i]))
		}
	}
}
...
func TestTanh() {
	for i := 0; i < len(vf); i++ {
		if f := Tanh(vf[i]); !veryclose(tanh[i], f) {
			panic(fmt.Sprintf("Tanh(%g) = %g, want %g", vf[i], f, tanh[i]))
		}
	}
	for i := 0; i < len(vftanhSC); i++ {
		if f := Tanh(vftanhSC[i]); !alike(tanhSC[i], f) {
			panic(fmt.Sprintf("Tanh(%g) = %g, want %g", vftanhSC[i], f, tanhSC[i]))
		}
	}
}
...
func TestTrunc() {
	for i := 0; i < len(vf); i++ {
		if f := Trunc(vf[i]); trunc[i] != f {
			panic(fmt.Sprintf("Trunc(%g) = %g, want %g", vf[i], f, trunc[i]))
		}
	}
	for i := 0; i < len(vfceilSC); i++ {
		if f := Trunc(vfceilSC[i]); !alike(ceilSC[i], f) {
			panic(fmt.Sprintf("Trunc(%g) = %g, want %g", vfceilSC[i], f, ceilSC[i]))
		}
	}
}
...
func TestY0() {
	for i := 0; i < len(vf); i++ {
		...
		if f := Y0(a); !close(y0[i], f) {
			panic(fmt.Sprintf("Y0(%g) = %g, want %g", a, f, y0[i]))
		}
	}
	for i := 0; i < len(vfy0SC); i++ {
		if f := Y0(vfy0SC[i]); !alike(y0SC[i], f) {
			panic(fmt.Sprintf("Y0(%g) = %g, want %g", vfy0SC[i], f, y0SC[i]))
		}
	}
}
...
func TestY1() {
	for i := 0; i < len(vf); i++ {
		...
		if f := Y1(a); !soclose(y1[i], f, 2e-14) {
			panic(fmt.Sprintf("Y1(%g) = %g, want %g", a, f, y1[i]))
		}
	}
	for i := 0; i < len(vfy0SC); i++ {
		if f := Y1(vfy0SC[i]); !alike(y1SC[i], f) {
			panic(fmt.Sprintf("Y1(%g) = %g, want %g", vfy0SC[i], f, y1SC[i]))
		}
	}
}
...
func TestYn() {
	for i := 0; i < len(vf); i++ {
		...
		if f := Yn(2, a); !close(y2[i], f) {
			panic(fmt.Sprintf("Yn(2, %g) = %g, want %g", a, f, y2[i]))
		}
		if f := Yn(-3, a); !close(yM3[i], f) {
			panic(fmt.Sprintf("Yn(-3, %g) = %g, want %g", a, f, yM3[i]))
		}
	}
	for i := 0; i < len(vfy0SC); i++ {
		if f := Yn(2, vfy0SC[i]); !alike(y2SC[i], f) {
			panic(fmt.Sprintf("Yn(2, %g) = %g, want %g", vfy0SC[i], f, y2SC[i]))
		}
		if f := Yn(-3, vfy0SC[i]); !alike(yM3SC[i], f) {
			panic(fmt.Sprintf("Yn(-3, %g) = %g, want %g", vfy0SC[i], f, yM3SC[i]))
		}
	}
}
...
func TestLargeCos() {
	...
	for i := 0; i < len(vf); i++ {
		...
		f2 := Cos(vf[i] + large)
		if !close(f1, f2) {
			panic(fmt.Sprintf("Cos(%g) = %g, want %g", vf[i]+large, f2, f1))
		}
	}
}
...
func TestLargeSin() {
	...
	for i := 0; i < len(vf); i++ {
		...
		f2 := Sin(vf[i] + large)
		if !close(f1, f2) {
			panic(fmt.Sprintf("Sin(%g) = %g, want %g", vf[i]+large, f2, f1))
		}
	}
}

func TestLargeSincos() {
	large := float64(100000 * Pi)
	for i := 0; i < len(vf); i++ {
		f1, g1 := sinLarge[i], cosLarge[i]
		f2, g2 := Sincos(vf[i] + large)
		if !close(f1, f2) || !close(g1, g2) {
			panic(fmt.Sprintf("Sincos(%g) = %g, %g, want %g, %g", vf[i]+large, f2, g2, f1, g1))
		}
	}
}
...
func TestLargeTan() {
	...
	for i := 0; i < len(vf); i++ {
		...
		f2 := Tan(vf[i] + large)
		if !close(f1, f2) {
			panic(fmt.Sprintf("Tan(%g) = %g, want %g", vf[i]+large, f2, f1))
		}
	}
}
...
func TestFloatMinMax() {
	for _, tt := range floatTests {
		s := fmt.Sprint(tt.val)
		if s != tt.str {
			panic(fmt.Sprintf("Sprint(%v) = %s, want %s", tt.name, s, tt.str))
		}
	}
}
