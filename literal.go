...
//
func NewLiteral(val exact.Value, typ types.Type, pos token.Pos, end token.Pos) *Literal {
	return &Literal{typ, val, pos, end}
}

// intLiteral returns an untyped integer literal that evaluates to i.
func intLiteral(i int64) *Literal {
	return NewLiteral(exact.MakeInt64(i), types.Typ[types.UntypedInt], token.NoPos, token.NoPos)
}
...
func nilLiteral(typ types.Type) *Literal {
	return NewLiteral(exact.MakeNil(), typ, token.NoPos, token.NoPos)
}
...
func zeroLiteral(t types.Type) *Literal {
	...
	case *types.Basic:
		...
		case t.Info()&types.IsBoolean != 0:
			return NewLiteral(exact.MakeBool(false), t, token.NoPos, token.NoPos)
		case t.Info()&types.IsNumeric != 0:
			return NewLiteral(exact.MakeInt64(0), t, token.NoPos, token.NoPos)
		case t.Info()&types.IsString != 0:
			return NewLiteral(exact.MakeString(""), t, token.NoPos, token.NoPos)
	...
	case *types.Pointer, *types.Slice, *types.Interface, *types.Chan, *types.Map, *types.Signature:
		return nilLiteral(t)
	case *types.Named:
		return NewLiteral(zeroLiteral(t.Underlying()).Value, t, token.NoPos, token.NoPos)
